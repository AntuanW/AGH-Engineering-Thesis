...
def test_find_all():
    ...
    expected_mapping = MappingModel(lab_group=mock_lab_group, mapped_devices=[
        MappedDeviceModel(
            name='R11',
            netmiko_device_type=NetmikoDeviceType('cisco_ios'),
            ip_address=IPvAnyAddress('172.17.145.10'),
    ...
    repo = MappingRepository()
    repo.find = MagicMock(return_value=mock_mapping)
    repo.find_one = MagicMock(return_value=mock_mapping[0])

    found_mappings = repo.find_all()

    assert found_mappings is not []
    assert found_mappings == [expected_mapping]
